...
BEGIN
    ...
    ALTER TABLE dq_dev..Temp_MainDailyDisbursement ADD debug_col VARCHAR(MAX);

    -- 2) Trigger TRUNCATE TABLE (security/data-loss)
    TRUNCATE TABLE dq_dev..Temp_MainDailyDisbursement;

    -- 3) Trigger DROP TABLE (warning/error tergantung rule)
    DROP TABLE dq_dev..Temp_MainDailyDisbursement;
END
/* ======================================================= */
